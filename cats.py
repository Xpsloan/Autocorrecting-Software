...
def about(subject):
    ...
    def sumn(lst):
        for i in subject:
            if i in split(lower(remove_punctuation(lst))):
                return True
        return False
    return sumn
...
def accuracy(typed, source):
    ...
    if not(ret):
        return 0.0
    return ret/len(typed_words) * 100
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for i in range(len(word_list)):
        if diff_function(typed_word, word_list[i], limit) < mi:
            mi = diff_function(typed_word, word_list[i], limit)
            ret = [word_list[i]] + ret
        elif diff_function(typed_word, word_list[i], limit) == mi:
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed == '' or source == '':  # Feel free to remove or add additional cases
        ...
        return min(1 + add, 1 + substitute, 1 + remove)
...
def report_progress(typed, prompt, user_id, upload):
    ...
    i = 0
    while i < len(prompt) and i < len(typed) and typed[i] == prompt[i]:
        progress += 1 
        i += 1
    progress /= len(prompt)
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        n = 0
        inner_list = []
        while n + 1 < len(times_per_player[i]):
            inner_list.append(abs(times_per_player[i][n] - times_per_player[i][n + 1]))
            n += 1
        list += [inner_list]
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    ret_lst = [[] for i in player_indices]
    
    lst = get_all_words(match)
    
    check_lst = [[] for i in word_indices]                                
    for x in word_indices:
        for i in player_indices:
            check_lst[x].append(time(match, i, x))
        
    for i in player_indices:
        for x in word_indices:
            if lst[x] != 0 and min(check_lst[x]) == time(match, i, x):
                ret_lst[i].append(get_word(match, x))
                lst[x] = 0
